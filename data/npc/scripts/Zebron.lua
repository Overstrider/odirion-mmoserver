...
if msgcontains(msg, 'gambl') or msgcontains(msg, 'Gambl') or msgcontains(msg, 'game') or msgcontains(msg, 'Game') or msgcontains(msg, 'dice') or msgcontains(msg, 'Dice') then
	npcHandler:say('So you care for a civilized game of dice?')
	zebron_talk_state = 1
			
elseif zebron_talk_state == 1 and msgcontains(msg, 'yes') or zebron_talk_state == 1 and msgcontains(msg, 'Yes') then
	npcHandler:say('I will roll a dice. If it shows 6, you will get five times your bet. How much do you want to bet?')
	zebron_talk_state = 2
...
if amount >= 1 and amount <= 99 then
	if doPlayerRemoveMoney(cid, amount) == TRUE then
		...
		npcHandler:say('Ok, here we go ... 6! You have won ".. amount*5 ..", congratulations. One more game?')
		zebron_talk_state = 1
		else
		npcHandler:say('Ok, here we go ... ".. CHANCE .."! You have lost. Bad luck. One more game?')
		zebron_talk_state = 1		
		end
	else
	npcHandler:say("I am sorry, but you don't have so much money. How much do you want to bet?")
	zebron_talk_state = 2
	end	
else	
npcHandler:say("I am sorry, but I accept only bets between 1 and 99 gold. I don't want to ruin you after all. How much do you want to bet?")	
zebron_talk_state = 2
end
