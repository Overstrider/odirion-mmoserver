...
local cfg = {
        ...
        npcHandler:say(''..getPlayerName(cid)..' ! HEHEHEHE! Another fool visits the tower! Excellent!.', cid)
        talk_state = 1
		...
		elseif msgcontains(msg, 'green') and talk_state == 11 then
            npcHandler:say('UHM UH OH ... How could you guess that? Are you mad??? All right. Penultimate question: What is the opposite?.', cid)
			talk_state = 12	
		elseif msgcontains(msg, 'none') and talk_state == 12 then
            npcHandler:say('NO! NO! NO! That cant be true. You\'re not only mad, you are a complete idiot! Ah well. Here is the last question: What is 1 plus 1?.', cid)
			talk_state = 13				
		elseif getPlayerStorageValue(cid, Storage.ParadoxTower.Lubo) == 1 and talk_state == 13 then
        if (getPlayerStorageValue(cid, Storage.ParadoxTower.Prisoner) >= 100) and msgcontains(msg, tostring(getPlayerStorageValue(cid, Storage.ParadoxTower.Prisoner))) then
			npcHandler:say('WRONG!!', cid)
			doTeleportThing(cid, cfg.toPos)
			talk_state = 0	
        else
			setPlayerStorageValue(cid, Storage.ParadoxTower.Riddler, 1)
			npcHandler:say('WRONG!!', cid)
