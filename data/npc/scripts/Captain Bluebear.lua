...
local voices = { {text = 'Passages to Carlin, Ab\'Dendriel, Edron, Venore.'} }
...
if msgcontains(msg, 'hi') then
	if focus == 0 then
		...
		npcHandler:say("Welcome on board, "..getCreatureName(cid)..". Where can I {sail} you today?", cid)  
		focus = cid
	else
		selfSay('Just a moment.', cid)
	end
...
function creatureDisapperCallback(cid, type, msg)
	if focus == cid then
          focus = 0
    end
return true
...
keywordHandler:addKeyword({'trip'}, StdModule.say, {npcHandler = npcHandler, text = 'Where do you want to go? To {Carlin}, {Ab\'Dendriel}, {Venore}?'})
keywordHandler:addKeyword({'route'}, StdModule.say, {npcHandler = npcHandler, text = 'Where do you want to go? To {Carlin}, {Ab\'Dendriel}, {Venore}?'})
keywordHandler:addKeyword({'passage'}, StdModule.say, {npcHandler = npcHandler, text = 'Where do you want to go? To {Carlin}, {Ab\'Dendriel}, {Venore}?'})
keywordHandler:addKeyword({'town'}, StdModule.say, {npcHandler = npcHandler, text = 'Where do you want to go? To {Carlin}, {Ab\'Dendriel}, {Venore} or {Edron}?'})
keywordHandler:addKeyword({'destination'}, StdModule.say, {npcHandler = npcHandler, text = 'Where do you want to go? To {Carlin}, {Ab\'Dendriel}, {Venore} or {Edron}?'})
keywordHandler:addKeyword({'sail'}, StdModule.say, {npcHandler = npcHandler, text = 'Where do you want to go? To {Carlin}, {Ab\'Dendriel}, {Venore} or {Edron}?'})
keywordHandler:addKeyword({'go'}, StdModule.say, {npcHandler = npcHandler, text = 'Where do you want to go? To {Carlin}, {Ab\'Dendriel}, {Venore} or {Edron}?'})
...
npcHandler:setCallback(CALLBACK_ONRELEASEFOCUS, creatureDisapperCallback)
