...
function creatureSayCallback(cid, type, msg)
	if not npcHandler:isFocused(cid) then
		...
		local queststate1 = getPlayerStorageValue(cid, Storage.ParadoxTower.Riddler)
		...
		elseif msgcontains(msg, 'yes') and talk_state == 3 then
            npcHandler:say('So know that everthing is based on the simple fact that 1 + 1 = '..random..'!', cid)				
				setPlayerStorageValue(cid, Storage.ParadoxTower.Prisoner, random)
                talk_state = 4
		elseif msgcontains(msg, 'bye') and (talk_state >= 1 and talk_state <= 4) then
			npcHandler:say('Good bye! Dont forget about the secrets of mathemagics..', cid)
