...
local guild_level = 5 -- Task Level / 5 = Slayer
local npclevel = TaskSystem.DifficultyLevel.Slayer
    ...
    if taskInfo then
        if getPlayerStorageValue(cid, taskInfo.storages.id) == TaskSystem.StorageValue.inTask then
            if getPlayerStorageValue(cid, taskInfo.storages.count) >= taskInfo.count then
                ...
                npcHandler:say("Here is your deeds from "..taskInfo.monsterList[1].." task.", cid)
            else
                npcHandler:say('You did not finished this task yet.', cid)
            end
        elseif getPlayerStorageValue(cid, taskInfo.storages.id) == TaskSystem.StorageValue.finishedTask then
            npcHandler:say('You already recive this deeds.', cid)
        end
...
keywordHandler:addKeyword(
    ...
    StdModule.say,
    {
        ...
        onlyFocus = true,
        text = "You want to do a {task}? or {leave}? can also {deeds} she ended up awards!"
    }
)
