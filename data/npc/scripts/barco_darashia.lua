...
local ThulePosition = {x=32983, y=31368, z=6}
local ThuleCost = 5000
local function creatureSayCallback(cid, type, msg)
	local talkUser = NPCHANDLER_CONVBEHAVIOR == CONVERSATION_DEFAULT and 0 or cid
	if not npcHandler:isFocused(cid) then
		return false
	end
	if (msgcontains(msg, 'bring') and msgcontains(msg, 'me') and msgcontains(msg, 'to') and msgcontains(msg, 'ankrahmun') and (not npcHandler:isFocused(cid))) then
		if doPlayerRemoveMoney(cid, 100) == TRUE then
		doTeleportThing(cid,{x=33092, y=32884, z=6})
    ...
    elseif msgcontains(msg, 'thule') then
		gthule = ''
		if getPlayerItemCount(cid, 7500) >= 1 then
			gthule = 'scroll'
			npcHandler:addFocus(cid)
			selfSay('You are sure that you wanna go to Thule payin\' a '..ItemType(7500):getName()..'?.')
			talkState[talkUser] = 2
		else
			if getPlayerMoney(cid) >= ThuleCost then
				gthule = 'money'
				npcHandler:addFocus(cid)
				selfSay('You are sure that you wanna go to Thule payin\' '..ThuleCost..'gps?.')
				talkState[talkUser] = 2
			else
				selfSay('Sorry, you don\'t have thule passage scroll or money enough.')
	...
	elseif talkState[talkUser] == 2 and msgcontains(msg, 'yes') then
		if gthule == 'money' then
			if doPlayerRemoveMoney(cid, ThuleCost) then
				doTeleportThing(cid,ThulePosition)
			...
			if getPlayerItemCount(cid, 7500) >= 1 then
				doPlayerRemoveItem(cid, 7500, 1)
				doTeleportThing(cid,ThulePosition)
				selfSay('See you later.')
				return true
			else
				selfSay('Sorry, you don\'t have thule passage scroll.')
				talkState[talkUser] = 0
				return true
			end
		end
	...
	if isPremium(cid) == TRUE then
		...
		doTeleportThing(cid,{x=32532, y=32784, z=6})
		npcHandler:addFocus(cid)
		else 
         selfSay('Sorry, you don\'t have enough money.') 
        end
...
local DestinationText = 'I can take you to {Venore} and {Ankrahmun}.'            
local CitysText = 'I can take you to {Venore} and {Ankrahmun}.' 
...
---------------------------------------THAIS CONFIG----------------------------------------------- 
local ThaisPosition = {x=32313, y=32212, z=6}               ----> Destination from Thais Boat <---- 
...
---------------------------------------PORT HOPE CONFIG------------------------------------------- 
local PortHopePosition = {x=32532, y=32784, z=6}         ----> Destination from Port Hope Boat <---- 
...
---Thule----------------------------------------------------------------------------------------------------------------------------------------- 
-- local ThuleText = 'Do you want to Travel to Thule for 5000 gold coins or Thule Passage Scroll?' 
-- local ThuleTextNo = 'Ok, come back when you want then!' 
-- local NoTravel = 'Sorry, i do not travel to this city..' 
-- local ThuleLvl = 8 
-- local ThulePremium = false --True/false 

-- Thule-- 
-- if Thule == true then 
-- local travelNode = keywordHandler:addKeyword({'thule'},  
-- StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = ThuleText }) 
-- travelNode:addChildKeyword({'yes'}, StdModule.travel, {npcHandler = npcHandler, premium = ThulePremium, level = ThuleLvl, cost = ThuleCost, destination = ThulePosition }) 
-- travelNode:addChildKeyword({'no'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, reset = true, text = ThuleTextNo }) 
-- else 
