...
if (getPlayerStorageValue(cid, Storage.ParadoxTower.Hugo) ~= 1) then
   return true
	...
	if msgcontains(msg, 'hello') then
        npcHandler:say('Welcome '.. getPlayerName(cid) ..' student of the arcane arts. I teach the fiercest spells available.', cid)
        talk_state = 1
		...
		elseif msgcontains(msg, 'crunors caress') and talk_state == 2 then
            npcHandler:say('A quite undruidic order of druids they were, as far as we know. I have no more enlightening knowledge about them though.', cid)
					setPlayerStorageValue(cid, Storage.ParadoxTower.Zoltan, 1)
					talk_state = 3
		elseif msgcontains(msg, 'bye') and (talk_state >= 1 and talk_state <= 3) then
			npcHandler:say('Use your knowledge wisely, '.. getPlayerName(cid) ..'!', cid)
			talk_state = 0
		end
