...
local function greetCallback(cid)
	if getPlayerStorageValue(cid,55874012) == 6 then
		--npcHandler:setMessage(MESSAGE_GREET, "Back again? How can I help you this time?", cid)
	else
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "pumin") then
		if getPlayerStorageValue(cid,55874012) == 2 then
	...
	elseif msgcontains(msg, "druid") or msgcontains(msg, "knight") or msgcontains(msg, "sorcerer") or msgcontains(msg, "paladin") then
		if npcHandlerfocus == 2 then
			npcHandler:say("I was a " .. vocation .. ", too, before I died!! What do you want from me?", cid)
			npcHandlerfocus = 3
		end
	elseif msgcontains(msg, "145") then
		if npcHandlerfocus == 3 then
			setPlayerStorageValue(cid,55874012, 3)
			npcHandler:say("That's right, you can get Form 145 from me. However, I need Form 411 first. Come back when you have it.", cid)
		elseif getPlayerStorageValue(cid,55874012) == 6 then
			setPlayerStorageValue(cid,55874012, 7)
