...
if (msgcontains(msg, "djanni'hah") or msgcontains(msg, "DJANNI'HAH")) and (not npcHandler:isFocused(cid)) then
	if getPlayerStorageValue(cid, Storage.DjinnWar.Faction.Greeting) == 1 then
		...
		npcHandler:say({"Whoa? You know the word! Amazing, "..getCreatureName(cid).."! ...",
						"I should go and tell Fa'hradin. ...",						
						"Well. Why are you here anyway, "..getCreatureName(cid).."?"}, cid)				
		talk_user = 1
	else 
        npcHandler:say("I have nothing more to say.", cid)      
	end
end
if msgcontains(msg, "Passage") and getPlayerStorageValue(cid, Storage.DjinnWar.MaridFaction.Start) ~= 1 and talk_user == 1 then
	npcHandler:say({"If you want to enter our fortress you have to become one of us and fight the Efreet. ... Umar: So, are you willing to do so?",
					"So, are you willing to do so?"}, cid)	
	talk_user = 2
elseif msgcontains(msg, "yes") and getPlayerStorageValue(cid, Storage.DjinnWar.MaridFaction.Start) ~= 1 and talk_user == 2 then
	...
	talk_user = 3
elseif msgcontains(msg, "yes") and getPlayerStorageValue(cid, Storage.DjinnWar.MaridFaction.Start) ~= 1 and talk_user == 3 then
	npcHandler:say({"Oh. Ok. Welcome then. You may pass. ...",
					"And don't forget to kill some Efreets, now and then."}, cid)
	setPlayerStorageValue(cid, Storage.DjinnWar.MaridFaction.Start, 1)	
	setPlayerStorageValue(cid, Storage.DjinnWar.Faction.Greeting, 2)
