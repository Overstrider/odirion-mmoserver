	...
	elseif npcHandler.topic[cid] == 1 then
		if msgcontains(msg, 'malor') then
			if efreet == 2 then
				...
				local lamps = player:addItem(2344, 1)
				doSetItemActionId(lamps.uid, 17898)
			elseif marid == 2 then
				...
				local lamps = player:addItem(2344, 1)
				doSetItemActionId(lamps.uid, 17899)
			end

			player:setStorageValue(Storage.DjinnWar.ReceivedLamp, 2)			
