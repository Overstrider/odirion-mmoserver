...
function getMarket()
	TABLESENDMARKET = {}
		local resultId = db.storeQuery("SELECT `name`, `category`, `itemClientId` FROM `market_items` ORDER BY `market_items`.`name` ASC")
		if (resultId ~= -1) then
			repeat	
				local name = result.getString(resultId, "name")
				local category = result.getNumber(resultId, "category")						
				local itemClientId = result.getNumber(resultId, "itemClientId")									
				table.insert(TABLESENDMARKET, {itemClientId, name, category})
...
function sendMarket(cid)
	local player = Player(cid)
		player:sendExtendedOpcode(220, 1)
		for i, v in pairs(TABLESENDMARKET) do
			player:sendExtendedOpcode(200, ""..v[1].."@"..v[2].."@"..v[3].."") 
		end
		player:sendExtendedOpcode(210, 1)
return true
end
