function setRate(rateName, value)
	...
	for line, str in pairs(tmp) do
		if string.find(str, rateName) then -- oque mudar
			tmp[line] = ""..rateName.." = "..value -- oque mudar e alterar
			break
		end
	end
...
function getRecord()
	local resultId = db.storeQuery("SELECT VALUE FROM `server_config` WHERE `config` LIKE 'players_record'")
...
function getPlayerFreeCap(cid) local p = Player(cid) return p ~= nil and math.floor((p:getFreeCapacity() / 100)) or false end
