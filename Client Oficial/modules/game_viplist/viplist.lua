...
function onAddVip(id, name, state, description, iconId, notify)
  ...
  connect(label, { onDoubleClick = function () g_game.openPrivateChannel(label:getText()) modules.game_console.addTab(label:getText(), true) return true end } )
...
function onVipListLabelMousePress(widget, mousePos, mouseButton)
  ...
  local menu = g_ui.createWidget('PopupMenu')
  menu:setGameMenu(true)
  menu:addOption(tr('Send Message'), function() g_game.openPrivateChannel(widget:getText()) modules.game_console.addTab(widget:getText(), true) end)
  menu:addOption(tr('Add new VIP'), function() createAddWindow() end)
