...
function UIMiniWindow:onDragLeave(droppedWidget, mousePos)
  ...
  if(modules.game_interface.getLeftPanel():isVisible()) then
    if mousePos.x >= modules.game_interface.getRightPanel():getRect().x and
    mousePos.x <= modules.game_interface.getRightPanel():getRect().x + modules.game_interface.getRightPanel():getRect().width then
      addEvent(function() self:setParent(modules.game_interface.getRightPanel()) end)
    elseif modules.game_interface.getRootPanel():getWidth() / 2 < mousePos.x then
		if(modules.game_interface.getExtraRightPanel():isVisible()) then
			addEvent(function() self:setParent(modules.game_interface.getExtraRightPanel()) end)
		else
			addEvent(function() self:setParent(modules.game_interface.getRightPanel()) end)
		end
    else
      addEvent(function() self:setParent(modules.game_interface.getLeftPanel()) end)
    end
  else
    if mousePos.x >= modules.game_interface.getRightPanel():getRect().x and
    mousePos.x <= modules.game_interface.getRightPanel():getRect().x + modules.game_interface.getRightPanel():getRect().width then
