...
function ProtocolGame.registerOpcode(opcode, callback)
  if opcodeCallbacks[opcode] then
    return false and error('opcode ' .. opcode .. ' already registered will be overriden')
  end
...
function ProtocolGame.registerExtendedOpcode(opcode, callback)
  if not callback or type(callback) ~= 'function' then
    return false and error('Invalid callback.')
  end

  if opcode < 0 or opcode > 255 then
    return false and error('Invalid opcode. Range: 0-255')
  end

  if extendedCallbacks[opcode] then
    return false and error('Opcode is not registered.')
  end
