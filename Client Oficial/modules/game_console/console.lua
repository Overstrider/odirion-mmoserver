  ...
  if mouseButton == MouseRightButton then
    ...
    if creatureName and #creatureName > 0 then
      if creatureName ~= g_game.getCharacterName() then
        menu:addOption(tr('Message to ' .. creatureName), function () g_game.openPrivateChannel(creatureName) modules.game_console.addTab(creatureName, true) end)
...
function doChannelListSubmit()
  ...
  if openPrivateChannelWith ~= '' then
    if openPrivateChannelWith:lower() ~= g_game.getCharacterName():lower() then
	  ...
	  modules.game_console.addTab(openPrivateChannelWith, true)	  
    else
