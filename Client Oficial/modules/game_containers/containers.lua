...
function onContainerOpen(container, previousContainer)
	...
	if previousContainer then
	    ...
	    elseif g_settings.get('defaultPanel') == 'extra' then
			if modules.game_interface.getExtraRightPanel():isVisible() then
				containerWindow = g_ui.createWidget('ContainerWindow', modules.game_interface.getExtraRightPanel())
			else
				containerWindow = g_ui.createWidget('ContainerWindow', modules.game_interface.getRightPanel())
			end
  ...
  refreshContainerPages(container)
	...
	local numLines = math.ceil(container:getCapacity() / 5)
	containerWindow:setContentMaximumHeight(cellSize.height * numLines * 1.)

	if not previousContainer then
		local filledLines = math.max(math.ceil(container:getItemsCount() / 5), 1)
