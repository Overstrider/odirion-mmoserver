...
void IOMapSerialize::loadHouseItems(Map* map)
{
	...
	DBResult_ptr result = Database::getInstance().storeQuery("SELECT `data` FROM `tile_store_alfa`");
	if (!result) {
		return;
	}
...
bool IOMapSerialize::saveHouseItems()
{
	...
	//clear old tile data
	if (!db.executeQuery("DELETE FROM `tile_store_alfa`")) {
		return false;
	}

	DBInsert stmt("INSERT INTO `tile_store_alfa` (`house_id`, `data`) VALUES ");
...
bool IOMapSerialize::loadHouseInfo()
{
	...
	DBResult_ptr result = db.storeQuery("SELECT `id`, `owner`, `paid`, `warnings` FROM `houses_alfa`");
	if (!result) {
		return false;
	}
	...
	result = db.storeQuery("SELECT `house_id`, `listid`, `list` FROM `house_lists_alfa`");
	if (result) {
		do {
...
bool IOMapSerialize::saveHouseInfo()
{
	...
	if (!db.executeQuery("DELETE FROM `house_lists_alfa`")) {
		return false;
	}
	...
	for (const auto& it : g_game.map.houses.getHouses()) {
		House* house = it.second;
		query << "SELECT `id` FROM `houses_alfa` WHERE `id` = " << house->getId();
		DBResult_ptr result = db.storeQuery(query.str());
		if (result) {
			query.str(std::string());
			query << "UPDATE `houses_alfa` SET `owner` = " << house->getOwner() << ", `paid` = " << house->getPaidUntil() << ", `warnings` = " << house->getPayRentWarnings() << ", `name` = " << db.escapeString(house->getName()) << ", `town_id` = " << house->getTownId() << ", `rent` = " << house->getRent() << ", `size` = " << house->getTiles().size() << ", `beds` = " << house->getBedCount() << " WHERE `id` = " << house->getId();
		} else {
			query.str(std::string());
			query << "INSERT INTO `houses_alfa` (`id`, `owner`, `paid`, `warnings`, `name`, `town_id`, `rent`, `size`, `beds`) VALUES (" << house->getId() << ',' << house->getOwner() << ',' << house->getPaidUntil() << ',' << house->getPayRentWarnings() << ',' << db.escapeString(house->getName()) << ',' << house->getTownId() << ',' << house->getRent() << ',' << house->getTiles().size() << ',' << house->getBedCount() << ')';
		}
	...
	DBInsert stmt("INSERT INTO `house_lists_alfa` (`house_id` , `listid` , `list`) VALUES ");
