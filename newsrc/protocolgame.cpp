...
void ProtocolGame::connect(uint32_t playerId, OperatingSystem_t operatingSystem)
{
	...
	player->registerCreatureEvent("secondCheck");
	
	player->registerCreatureEvent("ExtendedOpcode");
	// std::cout << "Testando Reconect2"<< std::endl;
...
bool ProtocolGame::startLiveCast(const std::string& password /*= ""*/)
{
	...
	if (isLiveCaster() || !player || player->isRemoved() || !connection || liveCasts.size() >= getMaxLiveCastCount()) {
		return false;
	}
...
void ProtocolGame::AddPlayerStats(NetworkMessage& msg)
{
	...
	msg.add<uint64_t>(std::min<uint64_t>(player->getExperience(), 0x7FFFFFFF));
	...
	msg.add<uint16_t>(std::min<uint32_t>(player->getMagicLevel(), std::numeric_limits<uint8_t>::max()));
